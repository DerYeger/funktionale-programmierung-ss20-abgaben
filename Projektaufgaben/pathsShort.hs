...
pathsShort n m sy sx ty tx z1 z2 z3 = map reverse $ iterate (concatMap netxSteps) [[Step sy sx sVal sVal]] !! (n * m - 1)
  where
    sVal = (arr !! sy) !! sx
    arr = buildArray n m z1 z2 z3
    netxSteps steps@(Step y x _ cScore : _) = foldl' validStep [] [(-1, 0, div), (0, -1, (-)), (1, 0, (*)), (0, 1, (+))]
      where
